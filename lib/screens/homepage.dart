...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _loadWeatherData() async {
    try {
      ...
      try {
        ...
        if (mounted) setState(() => _locationName = "My Location");
      }
  ...
  void _analyzeWeatherAndNotify(WeatherModel data) {
    bool isRaining = data.current.rain > 0.0;
    if (!isRaining) {
      int currentHour = DateTime.now().hour;
      if (currentHour + 1 < data.hourly.time.length) {
        if (data.hourly.precipitation[currentHour + 1] > 50.0) isRaining = true;
      }
    }
    ...
    bool isSiang = jamSekarang >= 6 && jamSekarang < 19;
        ...
        children: [
          ...
          ListView.separated(
            ...
            itemBuilder: (context, index) {
              int max = TempConverter.convert(
                value: daily.tempMax[index],
                from: "c",
                to: _selectedUnit,
              ).round();
              int min = TempConverter.convert(
                value: daily.tempMin[index],
                from: "c",
                to: _selectedUnit,
              ).round();
      ...
      child: GridView.count(
        ...
        children: [
          ...
          _buildConditionCard(
            ...
            subtitle: aqi < 51
                ? "Good"
                : (aqi < 101 ? "Moderate" : "Unhealthy"),
            visualContent: Icon(
              Icons.grain,
              color: aqi < 51
                  ? Colors.green
                  : (aqi < 101 ? kAccentYellow : Colors.redAccent),
              size: iconSize,
            ),
          ),
          _buildConditionCard(
            ...
            children: [
              Column(
                ...
                children: [
                  ...
                  Text(
                    WeatherUtils.formatTime(_weather!.daily.sunrise[0]),
                    style: const TextStyle(
                      color: kTextWhite,
                      fontSize: 20,
              ...
              Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 4),
                  Text(
                    WeatherUtils.formatTime(_weather!.daily.sunset[0]),
                    style: const TextStyle(
                      color: kTextWhite,
                      fontSize: 20,
                      fontWeight: FontWeight.w600,
  ...
  Widget _buildHourlyDetailsCard() {
    ...
    if (currentHour + count <= hourly.time.length) {
      for (int i = 0; i < count; i++) {
        if (dataToShow[currentHour + i] > maxVal)
          maxVal = dataToShow[currentHour + i];
      }
    }
          ...
          SizedBox(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, i) {
                int index = currentHour + i;
                bool isNow = i == 0;
                double val = dataToShow.length > index
                    ? dataToShow[index]
                    : 0.0;
                    ...
                    children: [
                      ...
                      Text(
                        isNow
                            ...
                            : WeatherUtils.formatTime(hourly.time[index]),
                        style: TextStyle(
                          color: isNow ? kTextWhite : kTextGrey,
                          fontSize: 12,
                          fontWeight: isNow
                              ? FontWeight.bold
                              : FontWeight.normal,
